...
@section('content_header')
    ...
    <h1>TAXONOMY DATA {{strtoupper(!empty($post_type_select_data)?$post_type_select_data->name:'')}}</h1>
...
<div class="box box-solid">
	<div class="box-header with-border">
		<form action="{{url()->full()}}" method="get" id="form-filter">
			  <div class="row">
		  	<div class="col-md-12">
		  		<a href="{{route('admin.taxonomy.create',['type_id'=>$post_type_select,'slug'=>Str::slug((!empty($post_type_select_data)?$post_type_select_data->name:''))])}}" class="btn btn-primary">TAMBAH TAXONOMY DATA {{strtoupper(!empty($post_type_select_data)?$post_type_select_data->name:'')}}</a>
		  		<hr>
		  	</div>
	   	<div class="col-md-4">
				...
				<tbody>
					@foreach ($data as $d)
						<tr>
							...
							<td>
								<div class="btn-group">
									<a href="{{route('admin.taxonomy.edit',['id'=>$d->id])}}" class="btn btn-primary btn-xs">Update</a>
									<button onclick="show_modal_delete('{{route('admin.taxonomy.delete',['id'=>$d->id])}}',{{$d->id}},'{{$d->name}}')"  class="btn btn-danger btn-xs"><i class="fa fa-trash"></i></button>
