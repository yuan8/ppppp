...
@section('content')
  ...
  <div class="box box-solid">
  	 <form method="POST" enctype='multipart/form-data' action="{{ route('admin.personil.update',['id'=>$data->id]) }}">
  	 	@csrf
  	 	@method('put')
        ...
        <input type="hidden" name="rand" value="{{$rand}}">
          ...
          <div class="form-group">
          <label>NRP</label>
          <input type="text" name="nrp" class="form-control" required="" value="{{$data->nrp}}">
        </div>
        <div class="form-group">
          <label>PANGKAT</label>
          <input type="text" name="pangkat" class="form-control" required=""  value="{{$data->pangkat}}">
        </div>
        <div class="form-group">
          <label>JABATAN</label>
            ...
            <div v-for="itm in items ">
                 <div class="row">
                  ...
                  <div class="col-md-3">
                    ...
                    <select class="form-control" v-bind:name="'pendidikan_umum['+itm.id+'][tag]'" required="" v-model="itm.tag">
                  ...
                  <div class="col-md-4">
                    <label>Label</label>
                    <input type="text" class="form-control" v-bind:name="'pendidikan_umum['+itm.id+'][label]'" v-model="itm.label">
                  </div>
                  <div class="col-md-3">
                    ...
                    <input type="file" class="form-control"  accept="image/*" v-bind:name="'pendidikan_umum['+itm.id+'][file]'">
                  </div>
               </div>
              <hr>
          ...
          <div  >
             <div v-for="itm in items ">
                 ...
                 <div class="col-md-3">
                    ...
                    <select class="form-control" v-bind:name="'pendidikan_bagian_personil['+itm.id+'][tag]'" required="" v-model="itm.tag" >
                  ...
                  <div class="col-md-4">
                    <label>Label</label>
                    <input type="text" class="form-control" v-bind:name="'pendidikan_bagian_personil['+itm.id+'][label]'" v-model="itm.label">
                  </div>
                  <div class="col-md-3">
                    <label>Ijazah/Sertifikat</label>
                       <input type="hidden" v-bind:name="'pendidikan_bagian_personil['+itm.id+'][file_recorded]'"  v-model="itm.file_recorded">
                    <input type="file" class="form-control"  accept="image/*" v-bind:name="'pendidikan_bagian_personil['+itm.id+'][file]'">
                  </div>
               </div>
            </div>
          </div>
  ...
  <script type="text/javascript">
      var pendidikan_umum=new Vue({
        ...
        data:{
          items:<?=$data->pendidikan_umum?json_encode($data->pendidikan_umum):'[]'?>
        },
        methods:{
          add_row:function(){
            this.items.push({
              'tag':null,
              'id':this.items.length==0?0:this.items.length,
              'label':null,
              'file_recorded':null
            });
          },
       ...
       var pendidikan_bagian_personil=new Vue({
        ...
        data:{
          items:<?=$data->pendidikan_bagian_personil?json_encode($data->pendidikan_bagian_personil):'[]'?>
        },
        methods:{
          add_row:function(){
            this.items.push({
       ...
       var data_lainya=new Vue({
        ...
        data:{
          items:<?=$data->order?json_encode($data->order):'[]'?>
        },
        methods:{
          add_row:function(){
            this.items.push({
